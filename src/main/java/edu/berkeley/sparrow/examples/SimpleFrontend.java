...
//import org.apache.log4j.FileAppender;//wyz
//import org.apache.log4j.PatternLayout;//wyz
//import org.apache.log4j.PropertyConfigurator;//wyz
...
import edu.berkeley.sparrow.daemon.util.Logging;//wyz
...
public class SimpleFrontend implements FrontendService.Iface {
  ...
  public static final String EXPERIMENT_MS = "experiment_ms";
  public static final int DEFAULT_EXPERIMENT_MS = 100;

  public static final String JOB_ARRIVAL_PERIOD_MILLIS = "job_arrival_period_millis";
  public static final int DEFAULT_JOB_ARRIVAL_PERIOD_MILLIS = 8;
  ...
  public static final String APPLICATION_NAME = "app_id";//wyz
  public static final String DEFAULT_APPLICATION_NAME = "sleepApp";//wyz
  String APPLICATION_ID = "";//wyz
  
  private static final Logger LOG = Logger.getLogger(SimpleFrontend.class);
  //private static final Logger AUDIT_LOG = Logging.getAuditLogger(SimpleFrontend.class);//wyz
  ...
  private class JobLaunchRunnable implements Runnable {
    ...
    public void run() {
      ...
      for (int taskId = 0; taskId < tasksPerJob; taskId++) {
        ...
        //wyz
        spec.setTaskId(APPLICATION_ID + Integer.toString(taskId));
  ...
  public void run(String[] args) {
    try {
      ...
      //PatternLayout p = new PatternLayout("%d{ISO8601} %p %c: %m%n");
      //FileAppender appender = new FileAppender(p, "~/sparrow/register.log", false);
      //LOG.addAppender(appender);//wyz
      ...
      //wyz
      APPLICATION_ID = conf.getString(APPLICATION_NAME, DEFAULT_APPLICATION_NAME);
      
      int arrivalPeriodMillis = conf.getInt(JOB_ARRIVAL_PERIOD_MILLIS,
          DEFAULT_JOB_ARRIVAL_PERIOD_MILLIS);
      int experimentDurationMs = conf.getInt(EXPERIMENT_MS, DEFAULT_EXPERIMENT_MS);
      LOG.debug("Using arrival period of " + arrivalPeriodMillis +
          " milliseconds and running experiment for " + experimentDurationMs + " millseconds.");
      ...
      //wyz
      LOG.info(APPLICATION_ID + " registerred to scheduler-svt at " + System.currentTimeMillis());
      //AUDIT_LOG.debug(Logging.auditEventString(APPLICATION_ID, " registerred to scheduler-svt"));
      ...
      LOG.debug("sleeping");
      while (System.currentTimeMillis() < startTime + experimentDurationMs) {
        Thread.sleep(5);
      }
